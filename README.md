...
1. **企業分析アシスタント**
    - OpenAIの最新アーキテクチャモデル（`gpt-5.2-pro`, `gpt-5.2`, `gpt-5-mini` 等）による高度な推論（Reasoning Effort対応）。
    - 履歴書作成支援、面接対策、有報比較など、用途に応じた複数のシステムプロンプト（モード）をワンタッチで切り替え。
2. **ナレッジベース管理 (RAG)**
...
```powershell
# 初回セットアップ & アプリケーションの起動（スプラッシュスクリーン表示）
uv run src/app.py
```
...
PyInstaller を用いて、Python環境が不要な単一の実行ファイル（exe）を作成することができます。

```powershell
# exeファイルの生成
uv run pyinstaller --noconsole --onefile --name syukatsu-support --collect-all src src/app.py -y
```
ビルドが完了すると、`dist` フォルダ内に `syukatsu-support.exe` が生成されます。
実行時（および配布時）は、必ずこのexeファイルと同じディレクトリに `config.json` および `.secret.key` を配置してください。
